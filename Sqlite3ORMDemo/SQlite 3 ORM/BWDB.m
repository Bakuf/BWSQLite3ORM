        ...
        for (int i = 0; i < param_count; i++) {
            ...
            if (o == nil) {
                ...
                if (strchr("islISLB", *[o objCType])) { // integer
                    ...
                    NSLog(@"bindSQL: Unhandled objCType: %s = %@", [o objCType],o);
                    statement = NULL;
                    return;
                }
...
- (NSNumber *) insertRow:(NSDictionary *) record {
    ...
    // NSLog(@"%s", __FUNCTION__);
    unsigned long dictSize = [record count];
    ...
    [self bindSQL:[query UTF8String] arguments:values];
    sqlite3_step(statement);
    if(sqlite3_finalize(statement) == SQLITE_OK) {
...
- (BOOL)checkIfColumnExists:(NSString*)column InTable:(NSString*)Table{
    ...
    //""
    bool boo = NO;
    NSMutableArray *tableProperties = [self getRowsFromQuery:[NSString stringWithFormat:@"pragma table_info (%@)",Table]];
    for (int i = 0; i < tableProperties.count; i++) {
        NSDictionary *row = tableProperties[i];
...
-(void)DisplayinLogContentofTable:(NSString*)TableName{
    ...
    NSArray *theArray = [self getAllRowsFromTable];
